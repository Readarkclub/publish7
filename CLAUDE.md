...
Claude 必须在每个响应的开头标出当前模式和模型。格式：`[模式：模式名称][模型：模型名称]`

### 模式 1: 研究
- **目的**：仅收集信息
- **允许**：阅读文件、提出澄清问题、理解代码结构
- **禁止**：建议、实施、计划或任何行动暗示
- **要求**：只能试图了解存在什么，而不是可能是什么。仅观察和提问。
...
- **禁止**：任何实现或代码、示例代码
- **要求**：计划须够全面，最后须转换为编号顺序的实施检查清单

### 模式 4: 执行
- **目的**：准确执行模式 3 中的计划
- **允许**：仅执行批准计划中明确详述的内容
- **禁止**：任何不在计划内的偏离、改进或创意添加
- **进入要求**：仅在明确发出 "进入执行模式" 命令后才能进入
...
- **NO SIMPLIFICATION** - 不允许简化，不要写 "//This is simplified stuff for now"
- **NO CODE DUPLICATION** - 检查现有代码库以重用函数和常量
- **NO DEAD CODE** - 要么使用，要么从代码库中完全删除
- **IMPLEMENT TEST FOR EVERY FUNCTION** - 为每个函数实现测试
- **NO CHEATER TESTS** - 测试必须准确、反映真实用法并设计来揭示缺陷
- **NO INCONSISTENT NAMING** - 遵循现有代码库的命名模式
- **NO OVER-ENGINEERING** - 不要添加不必要的抽象、工厂模式或中间件
- **NO MIXED CONCERNS** - 正确分离关注点
- **NO RESOURCE LEAKS** - 不要忘记关闭连接、清除超时、移除事件监听器
